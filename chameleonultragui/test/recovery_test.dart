...
void main() {
  ...
  test('Autopwn verifies the key from a weak Nested acquisition', () async {
    ...
    final found = await engine.recoverNestedSingle(
    ...
    expect(found, isTrue);
    expect(communicator.distanceRequests, 1);
    expect(communicator.nonceRequests, 1);
    expect(communicator.knownBlock, 3);
    expect(communicator.knownKeyType, 0x60);
    expect(communicator.targetBlock, 7);
    expect(communicator.targetKeyType, 0x61);
    expect(
      communicator.attemptedKeys.map(bytesToHex),
      contains('ffffffffffff'),
    );
    expect(engine.getSectorState(1, 1), ChameleonKeyCheckmark.found);
    expect(
  ...
  test(
    'Deep Autopwn ranks repeated Nested evidence before verification',
    () async {
      final communicator = _WeakNestedCard();
      final appState = ChameleonGUIState(SharedPreferencesProvider())
        ..log = Logger(level: Level.off)
      ...
      expect(found, isTrue);
      expect(communicator.nonceRequests, 5);
      expect(communicator.distanceRequests, 1);
      expect(
        communicator.attemptedKeys.map(bytesToHex),
        contains('ffffffffffff'),
  ...
  test(
    'Autopwn runs Static Nested fallback after the full weak pass',
    () async {
      final communicator = _WeakThenStaticFallbackCard();
      final appState = ChameleonGUIState(SharedPreferencesProvider())
        ..log = Logger(level: Level.off)
        ..connector = _UsbTestSerial()
        ..communicator = communicator;
      ...
      final engine = MifareClassicRecovery(
        ...
        mifareClassicType: MifareClassicType.mini,
        checkMarks: checkMarks,
      )..autopwnProgress = progress;
      engine.setKeyAsFound(0, 0, hexToBytes('A0A1A2A3A4A5'));

      await engine.recoverKeys();

      expect(communicator.weakRequests, 5);
      expect(communicator.staticRequests, 1);
      expect(
        progress.phase(AutopwnPhase.staticNested).status,
        AutopwnPhaseStatus.completed,
      );
      expect(engine.getSectorState(1, 1), ChameleonKeyCheckmark.none);
    },
    timeout: const Timeout(Duration(minutes: 2)),
  );

  test(
    'Static Nested fallback failure does not abort Autopwn',
    () async {
      final communicator = _WeakThenStaticFallbackCard(failStatic: true);
      final appState = ChameleonGUIState(SharedPreferencesProvider())
        ..log = Logger(level: Level.off)
        ..connector = _UsbTestSerial()
        ..communicator = communicator;
      ...
      await expectLater(engine.recoverKeys(), completes);
...
class _WeakNestedCard extends ChameleonCommunicator {
  _WeakNestedCard() : super(Logger(level: Level.off));

  final targetKey = hexToBytes('FFFFFFFFFFFF');
  final attemptedKeys = <Uint8List>[];
  int distanceRequests = 0;
  int nonceRequests = 0;
  int? knownBlock;
  int? knownKeyType;
  int? targetBlock;
  ...
  Future<NTDistance> getMf1NTDistance(
    ...
    return NTDistance(uid: 2374329723, distance: 613);
  }
  ...
  Future<NestedNonces> getMf1NestedNonces(
    ...
    return NestedNonces(
      nonces: [
        NestedNonce(nt: 1999585272, ntEnc: 3173333529, parity: 3),
        NestedNonce(nt: 128306861, ntEnc: 2363514210, parity: 7),
      ],
    );
  }
...
class _WeakThenStaticFallbackCard extends ChameleonCommunicator {
  _WeakThenStaticFallbackCard({this.failStatic = false})
    : super(Logger(level: Level.off));

  final bool failStatic;
  int weakRequests = 0;
  int staticRequests = 0;
  ...
  @override
  Future<NTLevel> getMf1NTLevel() async => NTLevel.weak;

  @override
  Future<DarksideResult> checkMf1Darkside() async => DarksideResult.fixed;
  ...
  Future<NTDistance> getMf1NTDistance(
    ...
    int keyType,
    Uint8List keyKnown,
  ) async => NTDistance(uid: 2374329723, distance: 13);
  ...
  Future<NestedNonces> getMf1NestedNonces(
    ...
    if (level == NTLevel.static) {
      staticRequests++;
      if (failStatic) {
        throw StateError('Static Nested is unavailable');
      }
      return NestedNonces(
        nonces: [
          NestedNonce(nt: 0x11111111, ntEnc: 0x22222222, parity: 0),
          NestedNonce(nt: 0x33333333, ntEnc: 0x44444444, parity: 0),
        ],
      );
    }
    if (level == NTLevel.hard) {
      return NestedNonces(
        nonces: [
          NestedNonce(nt: 0x01000000, ntEnc: 0x02000000, parity: 0),
          NestedNonce(nt: 0x03000000, ntEnc: 0x04000000, parity: 0),
        ],
      );
    }
    weakRequests++;
...
class _UsbTestSerial extends AbstractSerial {
  _UsbTestSerial() : super(log: Logger(level: Level.off)) {
